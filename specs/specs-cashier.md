...
- Табличка "КАССА" над столом
- Collision box для стола

## Patient Position at Cashier
- Позиция пациента у кассы: `(2.5, 0, -9.5)`
- Очередь за кассой: `(2.5, 0, -9.5 + index)` для index > 0
...
- Проверяет пересечение с мешами терминала
...
### Payment Flow
1. Пациент `atCashier` → игрок смотрит на терминал → подсказка
...
Game.Cashier.hasInteraction()  // → boolean (hover на терминал)
...
hoveredTerminal = false        // курсор на терминале
terminalMeshes = []            // меши терминала для raycasting
patientPos = Vector3(2.5, 0, -9.5)  // позиция пациента у кассы
```
...
- `shop.js`: проверяет `Game.Cashier.isPopupOpen()` перед открытием магазина
