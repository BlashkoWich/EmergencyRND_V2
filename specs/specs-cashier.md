...
- Баланс уменьшается при покупках в магазине

## Pricing (по тяжести заболевания)
```js
PRICES = { mild: 35, medium: 50, severe: 70 }
```
- Лёгкое (mild, startHp=80) → $35
- Среднее (medium, startHp=50) → $50
- Тяжёлое (severe, startHp=30) → $70

## Shop Cost
- Коробка препаратов (10 штук) стоит `$80`
- Коробка постельного белья (10 штук) стоит `$100`
...
- При наведении на **любую** часть кассовой стойки (терминал, стол, пациент):
  ...
  - Если пациент `atCashier`: подсказка `Game.Lang.t('cashier.hint.pay')`
  - Если пациента нет: подсказка `Game.Lang.t('cashier.hint.move')`
...
- Стилизация под реальный терминал банковской карты:
  ...
  - Экран: `Game.Lang.t('cashier.due')` + `Game.Lang.t('cashier.entered')`
...
5. Нажатие OK → `balance += amount`, HUD обновляется, начисляется XP (во время туториала XP копится, но level-up откладывается до завершения туториала)
...
Game.Cashier.getCurrentPatient()    // → текущий пациент у кассы (для Staff)
Game.Cashier.processPaymentAuto()   // автоматическая оплата (для Staff-кассира)
```

### Internal State
```js
balance = 350                  // текущий баланс
...
- `staff.js`: использует `getCurrentPatient()` и `processPaymentAuto()` для автоматической оплаты; проверяет `isStaffCashierHired()` для блокировки терминала
- `shift.js`: вызывает `spend(salary)` в конце дня для зарплаты сотрудников
- `index.html`: инициализация после `Game.Shelves.setup()`, обновление в animation loop

## Отрицательный баланс
- Баланс может уходить в минус (только через зарплату сотрудников)
- При `balance < 0`: цвет HUD `#ff4444` (красный), рамка красная
- Магазин по-прежнему блокирует покупки при нехватке средств
- `openTerminal()` блокируется если нанят Staff-кассир → `Game.Lang.t('cashier.staffWorking')`
