...
- `needsDiagnosis + правильный инструмент` → "ЛКМ — Диагностика ([название])"
- `needsDiagnosis + неправильный инструмент` → "Нужен инструмент ([нужный])"
- `needsDiagnosis + нет инструмента` → "Нужен инструмент ([нужный])"
- Диагностированный + нет препарата → "Нужен препарат ([нужный])"
- Диагностированный + правильный препарат → "ЛКМ — Применить [название]"

## Diagnosis Flow
1. ЛКМ на atBed пациента с правильным инструментом → `Game.Diagnostics.startMinigame(patient, type)`
...
- Инструменты: фонендоскоп ($1), рефлекс-молоток ($1), риноскоп ($1)
