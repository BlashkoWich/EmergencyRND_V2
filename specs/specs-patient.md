...
- При спавне волны появляется HUD-баннер (`#wave-banner`) с обратным отсчётом
- Текст: "Скорая привезла пациентов! Определите каждого — решите, кого спасать первым."
- Оранжево-красная полоска таймера, при <25% — красное свечение
...
## Health System
- `severe` → HP=30, `medium` → HP=50, `mild` → HP=80, max=100
