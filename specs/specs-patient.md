...
- Если новая волна спавнится пока баннер предыдущей ещё активен — баннер **перезапускается** для новой волны (таймер сбрасывается на `QUEUE_PATIENCE`, `activeWaveNumber` обновляется)
