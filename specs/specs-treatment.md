...
- `needsDiagnosis` → подсказка про инструмент
...
### ЛКМ на `atBed` пациента — прогрессивное лечение
- Пациент уже лечится (`treated === true`) → клик игнорируется
- `staffTreating === true` → "Медсестра уже лечит этого пациента"
- Нет препарата → "Выберите препарат в инвентаре"
- Правильный препарат (`pendingConsumables.indexOf(activeType) !== -1`):
...
- Неправильный препарат (`pendingConsumables.indexOf(activeType) === -1`):
