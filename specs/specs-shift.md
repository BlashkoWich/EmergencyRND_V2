...
- Содержит:
  - `#wave-text`: лорный текст (Game.Lang.t('wave.arrived')) — **виден всё время**
  - `#wave-timer-bar` + `#wave-timer-fill`: красно-оранжевая полоска обратного отсчёта — **скрыта до последних 15 сек**
  - `#wave-timer-text`: секунды (оранжевый моноширинный) — **скрыт до последних 15 сек**
- При `waveQueueTimer <= 15`: полоска и текст появляются, fill отсчитывает от 100% до 0% за 15 секунд
- Класс `.urgent` при <25% общего времени — красное свечение
- Показывается при спавне волны, скрывается когда все пациенты волны распределены или ушли
- При спавне новой волны поверх активного баннера — баннер **перезапускается** (таймер сброс на 60 сек, отслеживание переключается на новую волну)
