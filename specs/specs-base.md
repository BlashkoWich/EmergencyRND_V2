...
Порядок загрузки: helpers → world → patients → controls → consumables → inventory → shop → furniture → washing-machine → shelves → diagnostics → shift → cashier → inline module (оркестратор).
...
Создаёт renderer, scene, camera, collidables[]. Вызывает `Game.World.setup()`, `Game.Controls.setup()`, `Game.Furniture.setup()` + `registerExisting()`, `Game.WashingMachine.setup()`, `Game.Patients.setup()`, `Game.Consumables.setup()`, `Game.Inventory.setup()`, `Game.Shop.setup()`, `Game.Shelves.setup()`, `Game.Diagnostics.setup()`, `Game.Cashier.setup()`, `Game.Shift.setup()`. Запускает animation loop.
...
2. **Стиральная машина** — `Game.WashingMachine.hasInteraction()` (ЛКМ загрузка белья, E запуск стирки)
3. **Мебель** — `Game.Furniture.hasInteraction()` / `isCarrying()` (E для подъёма/размещения, ЛКМ для замены белья)
4. **Коробки** — `Game.Consumables.hasBoxInteraction()` (E для подбора)
5. **Расходники** — `Game.Consumables.hasInteraction()` (ЛКМ для подбора)
6. **Стеллажи** — `Game.Shelves.hasInteraction()`
7. **Касса** — `Game.Cashier.hasInteraction()`
8. **Табличка смены** — `Game.Shift.hasInteraction()` (самый низкий приоритет)
