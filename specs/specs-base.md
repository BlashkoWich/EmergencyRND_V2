...
Порядок загрузки: helpers → world → patients → controls → consumables → inventory → shop → furniture → washing-machine → shelves → tool-panel → diagnostics → staff → shift → cashier → inline module (оркестратор).
...
6. **Стеллажи** — `Game.Shelves.hasInteraction()` (только расходники, инструменты отклоняются с уведомлением)
7. **Панель инструментов** — `Game.ToolPanel.hasInteraction()` (только инструменты)
8. **Касса** — `Game.Cashier.hasInteraction()`
