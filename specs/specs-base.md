...
Порядок загрузки: helpers → world → patients → controls → consumables → inventory → shop → shelves → diagnostics → cashier → inline module (оркестратор).
...
js/
  ...
  shop.js               — магазин расходников (попап с табами: препараты + инструменты)
...
5. `Game.Cashier.update(delta)` — касса, терминал, очередь оплаты
...
| `shop-popup` | div | Попап магазина (табы: препараты + инструменты) |
...
2. **Коробки** — `Game.Consumables.hasBoxInteraction()` (E для подбора)
3. **Расходники** — `Game.Consumables.hasInteraction()` (ЛКМ для подбора)
4. **Стеллажи** — `Game.Shelves.hasInteraction()`
5. **Касса** — `Game.Cashier.hasInteraction()`
